...
public class CannisterAnimator : RhythmAnimator
{
    ...
    void Start()
    {
        ...
        {
            SetColor(CanType.DEFAULT);
            SetCanisterColor();            
        }

        string sequenceName = MusicManager.Instance._currentSequence.name;
        _colorName = sequenceName.Contains("Fire")      ? CanType.FIRE      :
                     sequenceName.Contains("Explosion") ? CanType.EXPLOSION :
                     sequenceName.Contains("Groovy")    ? CanType.GROOVY    :
                     CanType.DEFAULT;
    ...
    public void SetTypeToFire()
    {
        _material  = _cannisterRdr.material;
        _colorName = CanType.FIRE;
        SetCanisterColor();
    }

    public void SetColor(CanType cannisterType)
    {
        _colorName = cannisterType;
        SetCanisterColor();
    }
}
