    ...
    void UpdateContainers()
    {
        ...
        for (int i = 0; i < _currentContainerBeats.Count; i++)
        {
            if (_currentContainerBeats[i] < _beatsCanisterReticleDuration)
            {
                ...
                _attackReticles[i].transform.localScale = Vector2.MoveTowards(_attackReticles[i].transform.localScale*_reticleSize.x, 
                                                                              _attackReticles[i].transform.localScale*_reticleSize.y, _step);
            }
